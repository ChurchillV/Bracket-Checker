    ...
    for (let i = 0; i < input.length; i++) {
      ...
      if (this.isClosingBracket(input[i])) {
        ...
        if (
          ...
          this.completesBracket(bracketStack[0].bracket, input[i])
        ) {
        ...
        // 2 - Tell us the expected closing bracket
          ...
          return `Bracket "${input[i]}" doesn't match unclosed Bracket \"${
            bracketStack[0].bracket
          }\" at position \"${bracketStack[0].position}\"\n
                    Expected a \"${
                      this.openingBrackets[bracketStack[0].bracket]
                    }\"`;
        }
        ...
        return `Error: Closing bracket \"${
          input[i]
        }\" at position \"${i}\" with no opening bracket\n
                        Expected a "${
                          this.closingBrackets[input[i]]
                        }" before it`;
      }
    }
  ...
  document
    ...
    .addEventListener("input", function () {
      ...
      if (phrase.length > 0) {
        ...
        // var trimmedPhrase = phrase.replace(/\s/g, '').replace(/[^()[\]{}]/g, '');
        // var errorMessages = BracketChecker.checkBrackets(trimmedPhrase);
...
function validateBrackets(phrase) {
  ...
  // alert(filteredPhrase);
  // alert(filteredPhrase.length);
